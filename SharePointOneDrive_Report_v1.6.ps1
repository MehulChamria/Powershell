...
function Get-SharePointFiles{
    ...
    if ($email) {
        try {
            Set-Content $email_log_file -Value $null -ErrorAction Stop
            $email_interval = Get-date
            $email_style = “<style>table, th, td {border: 1px solid;}</style>”
        }
        catch {
            Write-Warning "Failed to create email log file"
            break
        }
    }
    $total_file_count = ($file_list.group).count
    ...
    $file_list | Group-Object SiteURL | ForEach-Object {
        $file_list = $_.Group
...
#Connect to SharePoint
        ...
        $file_list | ForEach-Object {
            ...
            if ($email) {
                if ((New-TimeSpan -Start $email_interval -End (Get-Date)).Minutes -ge $email_frequency) {
                    $email_interval = Get-Date
                    $email_statistics = [PSCustomObject]@{
                        "Time Stamp" = Get-Date
                        "Total Files" = $total_file_count
                        "Downloaded" = $downloaded_file_count
                        "Failed" = $failed_file_count
                        "Skipped" = $skipped_file_count
                        "Remaining" = $remaining_file_count
                        "Elapsed Time" = $download_elapsed_time
                    }
                    $email_statistics | Export-Csv -Path $email_log_file -Append -NoTypeInformation
                    [String]$message_body = Import-Csv $email_log_file | ConvertTo-Html -Head $email_style
                    $parameters = @{
                        To = $email_recipients
                        From = $email_sender
                        Subject = "STATUS UPDATE: Sharepoint files download status"
                        BodyAsHtml = $true
                        Body = $message_body
        ...
        Write-Verbose "Attempting to download failed files"
        $failed_file_list | ForEach-Object {
...
Get-SharePointFiles -filelist $file_list -logfile $downloads_log_file
