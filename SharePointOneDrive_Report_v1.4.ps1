...
#User ID of the user (Must be a Global Admin on SharePoint Online) running the script
...
#region Verify if the SharePointReport folder exists, if not, create it.
...
#region Verify if the Author list file exists, if not, exit execution.
...
Foreach ($item in $list_items) {
    ...
    $logs.Add("$timestamp - $item_counter of $($list_items.Count) - Exporting metadata from document $($output.Name)") | Out-Null
...
#region Connect to SharePoint
        ...
        $file_list | ForEach-Object {
            ...
            if(-not (Test-Path $filepath)) {
                ...
                $timestamp = get-date -Format "dd/MM/yyyy HH:mm:ss"
                Write-Verbose "$timestamp - $downloaded_file_count of $total_file_count - $($_.FileName) downloaded successfully" -Verbose
                Add-Content -Value "$timestamp - $downloaded_file_count of $total_file_count - $($_.FileRelativeURL) downloaded successfully" -Path $log_file
            }
            else {
                $downloaded_file_count++
                $timestamp = get-date -Format "dd/MM/yyyy HH:mm:ss"
                Add-Content -Value "$timestamp - $downloaded_file_count of $total_file_count - $($_.FileRelativeURL) already exists, skipping download." -Path $log_file
                Write-Verbose "$timestamp - $downloaded_file_count of $total_file_count - $($_.FileName) already exists, skipping download." -Verbose
            }
            $parameters = @{
                PercentComplete = (($downloaded_file_count / $total_file_count) * 100)
                Activity = "Downloading files from SharePoint"
                Status = "Successfully downloaded $downloaded_file_count of $total_file_count files."
            }
            Write-Progress @parameters
        }
    }
...
Write-Progress -Activity "Successfully dowloaded files from all sites" -Status "Completed" -Completed
#endregion